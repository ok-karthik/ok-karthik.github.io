...
- **Images**: this project builds with `output: 'export'` and `images.unoptimized: true`, so there is **no image optimizer at runtime** — `next/image` would add wrapper markup while optimizing nothing. Use plain `<img>`, and always set explicit `width`/`height` (prevents layout shift) plus `loading` and `decoding`. The `@next/next/no-img-element` lint rule is disabled for this reason; it assumes a server that this deployment doesn't have.
  - *(This corrected an earlier rule that mandated `next/image`. It was wrong for a static export and sent agents chasing a non-fix.)*
