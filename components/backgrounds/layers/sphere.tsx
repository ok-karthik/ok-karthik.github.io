...
export function createSphereLayer(mode: 'ai-core' | 'globe' | 'lattice' | 'swirl', id: string, label: string): Layer {
  ...
  class Particle {
    ...
    constructor(lx: number, ly: number, lz: number) {
      ...
      // Light mode: particles need to be darker to show up on light background
      const lightness = isLightMode ? (20 + Math.random() * 30) : (60 + Math.random() * 40)
    ...
    update(state: string, t: number) {
      ...
      const wave = Math.sin(t * 0.002 + this.latticePos.y * 0.01 + this.latticePos.x * 0.01) * 15
      ...
      if (state === 'lattice') {
          ...
          const tSec = t * 0.001;
          const u = this.aiTheta + tSec * 0.5;
          const wParam = Math.cos(this.aiPhi) * (coreRadius * 0.25);
          const thick = (this.aiRadius - coreRadius * 1.3) * 0.4;
          
          const R = coreRadius * 1.1; 
          ...
          const v_w = wParam * Math.cos(twist) - thick * Math.sin(twist);
          const v_thick = wParam * Math.sin(twist) + thick * Math.cos(twist);
          
          const jitterX = Math.sin(tSec * 15 + this.aiPhi * 10) * 1.5;
          const jitterY = Math.cos(tSec * 15 + this.aiTheta * 10) * 1.5;
          const jitterZ = Math.sin(tSec * 15 + this.aiPhi * 5) * 1.5;
  ...
  return {
    id,
    label,
    scope: 'hero', // sphere is scoped to hero
    setModes(modes: string[]) {
      currentModes = modes as typeof currentModes
      stateIndex = 0
      stateTime = 0
    },
    ...
    draw(ctx, t, pointer) {
      // Manage state cycling if there's a sequence
      stateTime++
      let activeMode = currentModes[stateIndex]
      
      if (currentModes.length > 1) {
        const duration = activeMode === 'ai-core' ? 600 : (activeMode === 'swirl' ? 350 : 500)
        if (stateTime > duration) {
          stateTime = 0
          stateIndex = (stateIndex + 1) % currentModes.length
        }
      }

      if (pointer.active) {
        const mouseX = (pointer.x / window.innerWidth) * 2 - 1
        const mouseY = (pointer.y / window.innerHeight) * 2 - 1
      ...
      // Offset the center to the lower-right of the hero
      // "Offset the canvas so the ribbon sits in the empty lower-right of the hero"
      const centerX = width * 0.75
      const centerY = height * 0.75
      
      const perspective = Math.min(width, height) * 2.5

      const projected: any[] = []

      particles.forEach(p => {
        const { prev, curr, didReset } = p.update(activeMode, t)
