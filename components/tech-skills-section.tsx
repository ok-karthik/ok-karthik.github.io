...
function SkillIcon({ skill, size = "md" }: { skill: Skill; size?: "sm" | "md" }) {
  ...
  if (Lucide) {
    return (
      <span
        className={`flex ${dim} shrink-0 items-center justify-center rounded-xl border border-border/70 bg-primary/10`}
      >
  ...
  if (skill.icon) {
    return (
      <span className={`flex ${dim} shrink-0 items-center justify-center rounded-xl bg-white/95 p-2 shadow-sm border border-border/40 overflow-hidden`}>
        <img
          src={skill.icon}
          alt=""
          width={size === "sm" ? 20 : 26}
...
export function TechSkillsSection() {
  return (
    <section id="tech-skills" className="py-20 px-6 relative z-20 scroll-mt-24">
      <div className="max-w-6xl mx-auto">
        ...
        {/* 3x3 Uniform Master Grid (9 Categories) */}
        <motion.div
                            ...
                            {skill.note && skill.note.length > 0 && (
                              <div className="mt-1.5 flex flex-wrap gap-1">
                                {skill.note.map((item) => (
                                  <span
                                    key={item}
                                    className="rounded-md border border-border/60 bg-secondary/80 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground"
                                  >
                                    {item}
                                  </span>
                                ))}
